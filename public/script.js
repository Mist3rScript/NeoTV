const app = {
    ...
    openEditChannel: (channel) => {
        ...
        if (channel.streams && channel.streams.length > 0) {
            container.innerHTML = channel.streams.map((s, idx) => {
                ...
                const providerType = s.provider_id ? 'xtream' : 'manual';
    ...
    getChannelById: async (id) => {
        const resp = await fetch(`${app.api}/admin/channels?page=1&limit=1000`);
        const data = await resp.json();
        return data.data.find(c => c.id == id);
    },

    setStreamPrimary: async (channelId, streamId) => {
        if (!confirm('Set as primary stream?')) return;
        try {
            await fetch(`${app.api}/admin/channels/${channelId}/primary/${streamId}`, { method: 'POST' });
            alert('Set as Primary: Done!');
            const ch = await app.getChannelById(channelId);
            app.openEditChannel(ch);
            app.loadChannels();
    ...
    deleteStream: async (streamId) => {
        ...
        try {
            await fetch(`${app.api}/admin/streams/${streamId}`, { method: 'DELETE' });
            alert('Stream deleted.');
            app.modal.close('editChannelModal'); // Force reload needed unless we refetch channel
            app.loadChannels();
        } catch (e) { alert('Error deleting stream'); }
    },
    ...
    navigate: (tabId) => {
        ...
        if (targetView) {
            ...
            console.warn(`View ID view-${tabId} not found.`);
        }
    },
        ...
        try {
            ...
            tbody.innerHTML = accounts.map(a => {
                ...
                <tr class="channel-row">
                    ...
                    <td><div style="font-size:0.9rem; font-weight:500; color:var(--primary);">${a.host_url || '-'}</div></td>
                    ...
                    <td>
                        <button class="btn-stream danger" onclick="app.deleteAccount(${a.id})"><i class="fas fa-trash"></i></button>
                    </td>
                </tr>
                `;
            }).join('');
    ...
    checkAccount: async (id, btn) => {
        ...
        try {
            ...
            if (data.status === 'DEAD') {
                ...
                alert('Connection Problem: ' + data.message);
            }
    ...
    loadCatalog: async () => {
        ...
        try {
            ...
            if (data.length === 0) {
                tbody.innerHTML = '<tr><td colspan="4" style="text-align:center; padding:20px; color:#666">No channels found.</td></tr>';
                return;
            }
    ...
    filterChannels: () => {
        ...
        rows.forEach(row => { // This might be unused now since we use folder view, but kept for legacy
    ...
    loadChannels: async (page = 1) => {
        ...
        try {
            ...
            let catLogos = {};
            try {
                const cResp = await fetch(`${app.api}/admin/categories`);
                if (cResp.ok) catLogos = await cResp.json();
            } catch (e) { }
            ...
            if (!data.data || data.data.length === 0) {
                container.innerHTML = '<div style="text-align:center; padding:40px; color:#666;">No channels available!</div>';
                return;
            }
            ...
            Object.keys(grouped).sort().forEach(category => {
                ...
                // Icon Logic
                let folderIconHtml = '<i class="fas fa-folder folder-icon"></i>';
                if (catLogos[category] && catLogos[category].length > 5) {
                    folderIconHtml = `<div class="folder-preview single"><img src="${catLogos[category]}"></div>`;
                }

                html.push(`<div class="folder-wrapper" style="display:flex; align-items:center; margin-bottom:5px;">`);
                // 1. Clickable Folder
                html.push(`<div class="folder-item" style="flex:1; display:flex; align-items:center; cursor:pointer;" onclick="app.toggleFolder('${folderId}')">`);
                html.push(folderIconHtml);
                html.push(`<span class="folder-name">${category}</span>`);
                html.push(`<span class="folder-count">${channels.length} items</span>`);
                html.push('<i class="fas fa-chevron-right folder-toggle"></i>');
                html.push('</div>');

                // 2. Separate Edit Button (Safe)
                const safeCat = category.replace(/'/g, "\\'");
                const safeLogo = (catLogos[category] || '').replace(/'/g, "\\'");
                html.push(`<button class="btn-icon" onclick="app.openEditCategory('${safeCat}', '${safeLogo}')" style="margin-left:8px; padding:8px; background:#222; border-radius:50%; width:32px; height:32px; display:flex; align-items:center; justify-content:center; border:1px solid #444; cursor:pointer;" title="Edit"><i class="fas fa-cog" style="font-size:14px; color:#aaa;"></i></button>`);
                html.push('</div>');

                html.push(`<div class="folder-channels" id="${folderId}">`);
                channels.forEach(c => {
                    const logo = c.logo_url ? `<img src="${c.logo_url}">` : '<i class="fas fa-tv" style="font-size:1.2rem; color:#444;"></i>';
                    html.push(`
    ...
    // --- NEW: Category Edit Functions ---
    openEditCategory: (name, logo) => {
        document.getElementById('editCatName').value = name;
        document.getElementById('editCatNameDisplay').value = name;
        document.getElementById('editCatLogo').value = (logo === 'undefined' || logo === 'null') ? '' : logo;
        app.modal.open('editCategoryModal');
    },

    saveCategory: async () => {
        const name = document.getElementById('editCatName').value;
        const logoUrl = document.getElementById('editCatLogo').value;
        try {
            await fetch(`${app.api}/admin/categories`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ name, logoUrl })
            });
            app.modal.close('editCategoryModal');
            app.loadChannels();
    ...
    toggleFolder: (folderId) => {
        ...
        if (folderEl) {
            const folderItem = folderEl.previousElementSibling; // Wrapper is sibling now? No, wrapper is parent.
            // Wait, previous element of folder-channels is the wrapper. 
            // The wrapper contains the .folder-item inside it.
            // Let's use simpler logic: toggle class on the ID directly.
            folderEl.classList.toggle('open');
            // Rotate chevron: find the chevron inside the wrapper.
            // Previous sibling of folder-channels IS .folder-wrapper.
            const wrapper = folderEl.previousElementSibling;
            if (wrapper) {
                const chevron = wrapper.querySelector('.folder-toggle');
                if (chevron) chevron.classList.toggle('rotate'); // Add CSS for rotate if needed, or stick to simple toggle
            }
        }
    },
    ...
    generateKey: async () => {
        ...
        if (!password) { alert('Admin password required'); return; }

        try {
            // FIX: Removed spaces from URL
        ...
        try {
            ...
            tbody.innerHTML = keys.map(k => {
                ...
                <tr class="channel-row">
                    ...
                    <td><span class="stream-badge ${k.is_active ? 'primary' : 'backup'}">${k.is_active ? 'Active' : 'Inactive'}</span></td>
                    <td>${k.duration_days} Days</td>
                    <td style="font-family:monospace; color:#aaa;">${k.device_id || 'Unused'}</td> <!-- FIX: closed tag -->
                    <td>${k.expiration_date ? new Date(k.expiration_date).toLocaleDateString() : '-'}</td>
                    <td><button class="btn-stream danger" onclick="app.deleteKey(${k.id})"><i class="fas fa-trash"></i></button></td>
                </tr>`;
            }).join('');
        } catch (e) { console.error(e); }
    },
    ...
    addChannel: async () => {
        ...
        if (!password || !name || !streamUrl) { alert('Please fill all fields'); return; }
    ...
    initCharts: () => {
        ...
        new Chart(ctx, {
            ...
            options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true, grid: { color: '#1a1a1a' } }, x: { grid: { display: false } } } }
        });
    }
};
